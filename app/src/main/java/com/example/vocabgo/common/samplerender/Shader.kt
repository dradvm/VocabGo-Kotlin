...
class Shader(
    ...
    companion object {
        ...
        fun createFromAssets(
            ...
            val assets: AssetManager = render.assetManager
            return Shader(
                render,
                Shader.inputStreamToString(
    ...
    init {
        ...
        try {
            vertexShaderId = createShader(GLES32.GL_VERTEX_SHADER, insertShaderDefinesCode(vertexShaderCode, definesCode))
            fragmentShaderId = createShader(GLES32.GL_FRAGMENT_SHADER, insertShaderDefinesCode(fragmentShaderCode, definesCode))
    ...
    fun setTexture(
        ...
        val uniform: Uniform? =
            uniforms.get(location)
        val textureUnit: Int
        if (uniform !is UniformTexture) {
            textureUnit = maxTextureUnit++
        } else {
            val uniformTexture: UniformTexture =
                uniform as UniformTexture
    ...
    fun setBoolArray(name: String, values: BooleanArray): Shader {
        val intValues = IntArray(values.size) { if (values[it]) 1 else 0 }
        ...
        try {
            ...
            for (entry in uniforms.entries) {
                try {
                    ...
                    val name = uniformNames.get(entry.key)
    ...
    private class UniformTexture(
        ...
        fun getTextureUnit(): Int = textureUnit

        override fun use(location: Int) {
            if (texture.textureId == 0) throw IllegalStateException("Tried to draw with freed texture")
            ...
            GLES32.glBindTexture(texture.target.glesEnum, texture.textureId)
            GLError.maybeThrowGLException("Failed to bind texture", "glBindTexture")
            GLES32.glUniform1i(location, textureUnit)
    ...
    private class UniformInt(private val values: IntArray) : Uniform {
        override fun use(location: Int) {
            GLES32.glUniform1iv(location, values.size, values, 0)
    ...
    private class Uniform1f(private val values: FloatArray) : Uniform {
        override fun use(location: Int) {
            GLES32.glUniform1fv(location, values.size, values, 0)
    ...
    private class Uniform2f(private val values: FloatArray) : Uniform {
        override fun use(location: Int) {
            GLES32.glUniform2fv(location, values.size / 2, values, 0)
    ...
    private class Uniform3f(private val values: FloatArray) : Uniform {
        override fun use(location: Int) {
            GLES32.glUniform3fv(location, values.size / 3, values, 0)
    ...
    private class Uniform4f(private val values: FloatArray) : Uniform {
        override fun use(location: Int) {
            GLES32.glUniform4fv(location, values.size / 4, values, 0)
    ...
    private class UniformMatrix2f(private val values: FloatArray) : Uniform {
        override fun use(location: Int) {
            GLES32.glUniformMatrix2fv(location, values.size / 4, false, values, 0)
    ...
    private class UniformMatrix3f(private val values: FloatArray) : Uniform {
        override fun use(location: Int) {
            GLES32.glUniformMatrix3fv(location, values.size / 9, false, values, 0)
    ...
    private class UniformMatrix4f(private val values: FloatArray) : Uniform {
        override fun use(location: Int) {
            GLES32.glUniformMatrix4fv(location, values.size / 16, false, values, 0)
