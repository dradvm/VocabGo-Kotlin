...
package com.example.vocabgo.third_party
...
class YuvToRgbConverter(context: Context) {
  ...
  private fun imageToByteArray(image: Image, outputBuffer: ByteArray, pixelCount: Int) {
    ...
    imagePlanes.forEachIndexed { planeIndex, plane ->
      ...
      val planeCrop =
        if (planeIndex == 0) imageCrop else Rect(imageCrop.left / 2, imageCrop.top / 2, imageCrop.right / 2, imageCrop.bottom / 2)
      val planeWidth = planeCrop.width()
      val planeHeight = planeCrop.height()
      val rowBuffer = ByteArray(plane.rowStride)
      val rowLength = if (pixelStride == 1 && outputStride == 1) planeWidth else (planeWidth - 1) * pixelStride + 1
      var offset = outputOffset
      for (row in 0 until planeHeight) {
        planeBuffer.position((row + planeCrop.top) * rowStride + planeCrop.left * pixelStride)
        if (pixelStride == 1 && outputStride == 1) {
          planeBuffer.get(outputBuffer, offset, rowLength)
          offset += rowLength
        } else {
          planeBuffer.get(rowBuffer, 0, rowLength)
          for (col in 0 until planeWidth) {
            outputBuffer[offset] = rowBuffer[col * pixelStride]
            offset += outputStride
          }
        }
      }
